...
export default function MyApp() {
  ...
  const runShowDash = () => {
    if (!loggedIn) {
      alert('Please log in as a user to access the customer dashboard.');
      runShowLogin();
      return;
    } else {
      setShowFirstPage(false);
  ...
  return (
    ...
    >
          ...
          {!loggedIn ? (
            ...
            <Button
              ...
              onClick={() => {
                setLoggedIn(false);
                setUsername('');
                setCart([]); // Clears the cart for the user
                setShowFirstPage(true);
      ...
      <Drawer anchor="left" open={drawerOpen} onClose={toggleDrawer(false)}>
        <Box
          sx={{ width: 250, backgroundColor: 'lightgreen', height: '100%', color: 'white' }}
          role="presentation"
